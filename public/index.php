...
$router->get('#^/(fr|en)/([a-z0-9-]+)/article/(\d{4})/(\d{2})/(\d{2})/(\d+)[_-]([a-z0-9-]+)(?:\.html)?/?$#', static function (
	string $lang,
	string $categorySlug,
	string $year,
	string $month,
	...
	$currentPath = sprintf('/%s/%s/article/%s/%s/%s/%s_%s.html', $lang, $categorySlug, $year, $month, $day, $id, $slug);
